...
bool Unit::checkDistanceCollision(Unit* pUnit)
{
	float	dX = this->getLocation().getX() - pUnit->getLocation().getX(),
			dY = this->getLocation().getY() - pUnit->getLocation().getY(),
			distSq = (dX * dX) + (dY * dY),
			radiusThis = ((float)(this->getCurrentSprite()->getWidth()) * .80f),
			radiusThat = ((float)(pUnit->getCurrentSprite()->getWidth()) * .80f),
